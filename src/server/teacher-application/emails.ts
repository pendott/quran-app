...
export async function sendTeacherApplicationApprovedEmail(params: {
  to: string;
  name: string;
  password: string;
}) {
  ...
  const html = emailLayout(`
    ...
    <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#334155;">Hi ${name},</p>
    <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#334155;">
      Welcome aboard! Your jomngaji.my teacher account is ready.
    </p>
    <div style="margin:0 0 16px;padding:12px 14px;border:1px solid #e2e8f0;border-radius:12px;background:#f8fafc;">
      <p style="margin:0 0 8px;font-size:14px;color:#334155;"><strong>Login email:</strong> ${escapeHtml(params.to)}</p>
      <p style="margin:0;font-size:14px;color:#334155;"><strong>Temporary password:</strong> ${password}</p>
    </div>
    ...
    <p style="margin:0;font-size:14px;line-height:1.6;color:#64748b;">Please change this password after your first sign in.</p>
  `);
